...
-- =========================================================
-- USER ROLES (separate table per security guideline)
-- =========================================================
...
CREATE TABLE public.settings (
  ...
  tiktok text DEFAULT '',
  working_days int[] NOT NULL DEFAULT ARRAY[0,1,2,3,4,5,6], -- 0=Sun..6=Sat
...
INSERT INTO public.settings (id, shop_name, whatsapp) VALUES (1, 'حلاق الباشا', '+201018172606')
...
BEGIN
  IF NEW.raw_user_meta_data ? 'full_name' AND NEW.raw_user_meta_data ? 'phone' THEN
    ...
    ON CONFLICT (id) DO NOTHING;
    -- default role = customer (unless metadata says otherwise)
